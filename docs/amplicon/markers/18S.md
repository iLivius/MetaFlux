...
!!! note "PR2 also carries prokaryotic and organellar references, which is useful"
    ...
    This is a feature rather than contamination of the reference. Eukaryotic primers
    amplify bacterial SSU and organellar rRNA more often than is comfortable, and because
    PR2 can name those products they are assigned rather than silently forced onto the
    nearest eukaryotic reference. `keep: [d__Eukaryota]` then removes them cleanly — so
    the filter is doing real work, not applying a formality.
...
!!! warning "V9 must be set manually"
    ...
    `trunc_len.min_read_coverage_pct` should absorb this, since it caps the ceiling at
    the length 95% of reads reach and after trimming that is the amplicon length itself.
