...
# Paths and settings come from rule finalize_otu_table in 45_abundance.smk:
#   input.raw_biom   = kraken-biom's raw table (rule kraken_biom folding every
#                      sample's Bracken report into one BIOM file)
#   output.biom/.tsv = the two files this script writes. Both are terminal
#                      outputs listed in _shotgun_targets() (00_common.smk) —
#                      requested directly by `rule all`, nothing further in the
#                      pipeline reads them back
#   params.*         = the shotgun.taxonomy_filter settings resolved in
#                      00_common.smk (OTU_FILTER_ENABLED/KEEP/DISCARD); off
#                      unless the run config turns it on
...
def logmsg(msg: str) -> None:
    """Write one line to the log file, flushing immediately so the log stays
    readable if the job dies partway through (e.g. filter removes everything)."""
...
def _clean_sample(sid: str) -> str:
    """kraken-biom takes each sample's id from its Bracken report filename
    ({sample}_report.txt, written by rule bracken), so every sample id in the
    raw BIOM ends in "_report". Strip that back to the plain sample name used
    everywhere else in MetaFlux (fastq basenames, other stats tables)."""
    s = str(sid)
...
def _normalize(tax_list) -> list:
    """Rebuild the Genus+species binomial in the species slot where it's safe
    to (see the block comment above for exactly when that is and why)."""
    genus = None
    for seg in tax_list:
        seg = str(seg).strip()
...
# Same idea as the amplicon taxonomy filter (workflow/scripts/amplicon/
# 80a_assign_taxonomy.R): a keep/discard token such as "g__Bacillus" is matched
# against a whole rank segment, not against the taxonomy as one blob of text —
# so it only matches an OTU whose genus segment IS "g__Bacillus", and can never
# accidentally match inside a longer name that merely contains that word.
# keep runs first (drop any OTU that matches none of the keep tokens), then
# discard (drop any OTU that matches any discard token) — an OTU has to pass
# both gates to survive.
def _passes(tax_list) -> bool:
    """True if this OTU's taxonomy (tax_list, e.g. ['k__Bacteria', ...,
    'g__Achromobacter', 's__Achromobacter sp. MFA1 R4']) passes the
    keep/discard filter described above."""
...
# The second argument to to_hdf5 is just a free-text "generated by" tag stored
# in the BIOM file's metadata (who/what produced it) — required by the biom
# API, but nothing reads it back.
